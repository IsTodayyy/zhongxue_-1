...
$(function(){
	...
	//修改学生信息
	$("#xg_xs_btn").click(function() {
		var xg_xs_mingzi = $("#xg_xs_mingzi").val();
		var xg_xs_name = $("#xg_xs_name").val();
		var xg_xs_password = $("#xg_xs_password").val();
		var xg_xs_banji = $("#xg_xs_banji").val();
		
		if(xg_xs_name.length > 10 || xg_xs_name.length < 6){
			alert("用户名长度应该在6至10位");
			return;
		}
		...
		if(xg_xs_password.length > 30 || xg_xs_password.length < 6){
			alert("密码长度应该在6至30位");
			return;
		}
		...
		$.ajax({
            type: "POST",
            url: locationUrl + "/xiugaixueshengxinxi",
            data: "xg_xs_mingzi="+ xg_xs_mingzi + "&xg_xs_name=" + xg_xs_name+ "&xg_xs_password=" + xg_xs_password+ "&xg_xs_banji=" + xg_xs_banji+ "&id=" + xiugaixueshengid,
            success: function(data){
            	if(data > 0){
            		alert("修改成功");
            	}else{
